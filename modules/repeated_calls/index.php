...
function listadoIVRCategory($pDB, $smarty, $module_name, $local_templates_dir){
    ...
    $date_end   = date('Y-m-d H:i:s',strtotime($paramFiltro['date_end'].":59"));
     ...
     $recordset = $oCalls->getSummery($date_start,$date_end);
	...
	$phone = $record[0];
	if($record[0][0] == '+')
		$phone = substr($record[0],1);


    $recordset[$in][0] = "<a target='_blank' href=index.php?menu=calls_detail&date_start={$sdate}&date_end={$edate}&phone={$phone}>{$record[0]}</a>";
    ...
    $arrColumnas = array( 'Caller ID','Count' );

    $oGrid->setColumns($arrColumnas);
    $oGrid->setTitle(_tr('Repeated Calls'));
    $oGrid->pagingShow(true);
    $oGrid->setNameFile_Export(_tr('Repeated Calls'));
