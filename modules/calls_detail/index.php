...
function reportCallsDetail($smarty, $module_name, $pDB, $local_templates_dir)
{
    ...
    foreach (array('date_start', 'date_end', 'calltype', 'agent', 'queue',
     ...
     if (!isset($paramFiltro['date_start'])) $paramFiltro['date_start'] = date("d M Y 00:00");
     if (!isset($paramFiltro['date_end'])) $paramFiltro['date_end'] = date("d M Y 23:59");
	 ...
	 $urlVars = array_merge($urlVars, $paramFiltro);
        ...
        $paramLista['date_start'] = date('Y-m-d H:i:s',strtotime($paramFiltro['date_start'].":00"));
        $paramLista['date_end']   = date('Y-m-d H:i:s',strtotime($paramFiltro['date_end'].":59"));
    ...
    if($bIssabelNuevo) {
         $oGrid->addFilterControl(_tr("Filter applied: ")._tr("Start Date")." = ".$paramLista['date_start'].", "._tr("End Date")." = ".
            $paramLista['date_end'], $paramLista, array('date_start' => date("d M Y 00:00"),'date_end' => date("d M Y 23:59")),true);
