...
function listAgent($pDB, $smarty, $module_name, $local_templates_dir)
{
    ...
    $arrColumns = array('', _tr("Number"), _tr("Name"), _tr("Status"),_tr("Email"),_tr("Options"));
...
function formEditAgent($pDB, $smarty, $module_name, $local_templates_dir, $id_agent){
    ...
    if (is_null($id_agent)) {
        ...
        if (!isset($_POST['email']))        $_POST['email'] = $arrAgente['email'];
    ...
    if ($bDoCreate || $bDoUpdate) {
        if(!$oForm->validateForm($_POST)) {
            ...
            foreach (array('extension', 'password1', 'password2', 'description', 'eccpwd1', 'eccpwd2','email') as $k)
