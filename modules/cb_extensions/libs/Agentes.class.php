...
class Agentes
{
    ...
    function getAgents($id=null)
    {
        ...
        $sQuery =
            'SELECT id, type, number, name, password, estatus, eccp_password,email '.
    ...
    function addAgent($agent)
    {
        ...
        $sPeticionSQL = 'INSERT INTO agent (type, number, password, name, eccp_password,email) VALUES (?, ?, ?, ?, ?,?)';
        $paramSQL = array($typeExtension[0], $typeExtension[1], $agent[1], $agent[2], $agent[3],$agent[4]);
    ...
    function editAgent($agent)
    {
        ...
        $sPeticionSQL = 'UPDATE agent SET password = ?, name = ?, type = ?,email = ?';
        $paramSQL = array($agent[1], $agent[2], $typeExtension[0],$agent[4]);
        if (!is_null($agent[3])) {
