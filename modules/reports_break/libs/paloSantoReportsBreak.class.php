...
class paloSantoReportsBreak
{
     ...
     */
    function getReportesBreak($fecha_init,$fecha_end,$agent_number)
    {

	if(!empty($agent_number))
		$agent_cond = "AND agent.number='{$agent_number}'";
        $sPeticionSQL = "
...
FROM agent 
 JOIN (audit, break) 
    ON (agent.id = audit.id_agent AND break.id = audit.id_break 
        AND audit.datetime_init >= '{$fecha_init}' AND audit.datetime_init <= '{$fecha_end}'
	{$agent_cond}
	)
